...
const run = async (searchWord, scroll, fileName) => {
  ...
  for (let img of imgs) {
    ...
    await img.click();
    await sleep(3)
    let imgurl = await driver.findElement(By.xpath(`//*[@id="Sva75c"]/div[2]/div/div[2]/div[2]/div[2]/c-wiz/div[2]/div[1]/div[1]/div[2]/div/a/img`)).getAttribute("src")
    if (imgurl != null) {
      links.push(imgurl);
      // Folder Path
